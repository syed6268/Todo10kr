...
export default function Dashboard() {
  ...
  const fetchGCalEvents = useCallback(async () => {
    ...
    try {
      ...
      setCalendarEvents(
        data.events.map((e, i) => ({ ...e, id: e.id || `gcal-${i}` }))
      )
  ...
  const insertIntoGoogleCalendar = async () => {
    ...
    try {
      ...
      if (data.needsReconnect && data.inserted === 0) {
        ...
        setError(
          `Google denied write access (${firstErr}). Disconnect, then Connect Google Calendar again to grant write permission.`
        )
      ...
      if (err.data?.needsReconnect) {
        setError(
          `Google denied write access (${err.message}). Disconnect, then Connect Google Calendar again.`
        )
      } else {
        setError(err.message)
      }
    ...
    <div className="flex flex-col gap-6">
      <header className="text-white">
        ...
        <p className="mt-1 text-sm text-indigo-100">
          Goal agents propose, the orchestrator builds your day.
        </p>
      </header>
      ...
      <section className="rounded-2xl bg-white p-6 shadow-xl shadow-black/10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-xl font-semibold text-indigo-600">Google Calendar</h2>
            <p className="mt-1 text-sm text-slate-500">
              {gcalConnected
                ? 'Connected — your real events can drive scheduling.'
                : 'Connect to pull today\u2019s actual events.'}
            </p>
          </div>
        ...
        <div className="mt-3 flex items-center gap-2 text-sm">
          ...
          >
            Use live Google Calendar events when generating
          </label>
        </div>
      </section>

      <section className="rounded-2xl bg-white p-6 shadow-xl shadow-black/10">
        <h2 className="text-xl font-semibold text-indigo-600">Calendar Events</h2>
        <p className="mt-1 text-sm text-slate-500">
          Events the AI must work around. Add manually or pull from Google.
        </p>

        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_auto]">
          <input
            type="text"
          ...
          {calendarEvents.map((event) => (
            ...
            >
              ...
              <button
                ...
                className="flex h-7 w-7 items-center justify-center rounded-full bg-red-500 text-lg leading-none text-white transition hover:bg-red-600"
                aria-label="Remove"
              >
                ×
              </button>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <section className="rounded-2xl bg-white p-6 shadow-xl shadow-black/10">
          <h2 className="text-lg font-semibold text-slate-800">Dump Todos</h2>
          <p className="text-sm text-slate-500">Everyday tasks</p>
          <ul className="mt-3 divide-y divide-slate-100">
            {dumpTodos.length === 0 && (
              <li className="py-3 text-sm text-slate-400">No dump todos yet.</li>
            )}
            {dumpTodos.map((todo) => (
              <li
                key={todo._id}
                className={`flex items-center justify-between py-2.5 ${todo.completed ? 'opacity-50' : ''}`}
              >
                <span
        ...
        <section className="rounded-2xl bg-white p-6 shadow-xl shadow-black/10">
          <h2 className="text-lg font-semibold text-slate-800">Suggested Todos</h2>
          <p className="text-sm text-slate-500">Growth & improvement seed items</p>
          <ul className="mt-3 divide-y divide-slate-100">
            {suggestedTodos.length === 0 && (
              <li className="py-3 text-sm text-slate-400">No suggested todos yet.</li>
            )}
            {suggestedTodos.map((todo) => (
              <li key={todo._id} className="py-2.5">
      ...
      <section className="rounded-2xl bg-white p-6 shadow-xl shadow-black/10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-xl font-semibold text-indigo-600">AI-Optimized Schedule</h2>
            <p className="mt-1 text-sm text-slate-500">
              Goal agents propose, the orchestrator builds, you tweak.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              ...
              onClick={generate}
              disabled={loading}
              className="rounded-lg bg-gradient-to-r from-indigo-600 to-violet-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/30 transition hover:from-indigo-700 hover:to-violet-700 disabled:opacity-60"
            >
            ...
            {schedule.length > 0 && (
              <button
                ...
                className="rounded-lg border-2 border-indigo-600 bg-white px-5 py-2.5 text-sm font-semibold text-indigo-600 transition hover:bg-indigo-50 disabled:opacity-50"
              >
                {pushingToGCal ? 'Inserting...' : `📅 Insert (${pushableCount})`}
              </button>
            )}
          </div>
        </div>
        ...
        {stats && (
          <div className="mt-5 grid grid-cols-2 gap-3 rounded-xl border border-fuchsia-200 bg-gradient-to-br from-fuchsia-50 to-pink-50 p-4 lg:grid-cols-4">
            {[
              ['Free Slots', stats.freeSlots],
              ['Free Time', `${stats.totalFreeMinutes ?? 0} min`],
              ['Dump Scheduled', stats.dumpScheduled || 0],
              ['Goal Tasks', stats.suggestedScheduled || 0],
            ].map(([label, value]) => (
              <div key={label}>
                <p className="text-xs font-medium text-slate-500">{label}</p>
                <p className="mt-0.5 text-xl font-bold text-indigo-600">{value}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {activeGoals.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2">
            {activeGoals.map((g) => (
              <span
                key={g._id}
                className="inline-flex items-center gap-1.5 rounded-full bg-violet-100 px-3 py-1 text-xs font-medium text-violet-700"
              >
                🎯 {g.title}
                <span className="opacity-60">· P{g.priority}</span>
              </span>
            ))}
          </div>
        )}
        ...
        {summary && (
          <div className="mt-5 rounded-lg border-l-4 border-indigo-500 bg-indigo-50/80 px-4 py-3 text-slate-700">
        ...
        {schedule.length === 0 && !loading && !error && (
          <p className="mt-6 text-center text-sm italic text-slate-400">
            Click Generate to let the orchestrator + goal agents build your day.
          </p>
        )}
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      ...
                      {slot.goalTitle && (
                        <span className="inline-block rounded-full bg-violet-100 px-2.5 py-0.5 text-[10px] font-bold text-violet-700">
                          🎯 {slot.goalTitle}
                        </span>
                      )}
                      {slot.gcalInserted && (
        ...
        {deferred.length > 0 && (
          <details className="mt-5 rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-600">
            <summary className="cursor-pointer font-medium text-slate-700">
              {deferred.length} task(s) deferred by the orchestrator
            </summary>
        ...
        {proposals.length > 0 && (
          <details className="mt-3 rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-600">
            <summary className="cursor-pointer font-medium text-slate-700">
              Goal agent proposals ({proposals.length})
            </summary>
            <div className="mt-3 space-y-3">
              {proposals.map((p) => (
                <div key={p.goalId} className="rounded border border-slate-200 bg-white p-3">
                  <p className="font-semibold text-slate-800">🎯 {p.goalTitle}</p>
                  {p.progressReport && (
                    <p className="mt-1 text-xs text-slate-500">{p.progressReport}</p>
                  )}
                    ...
                    {(p.candidates || []).map((c, i) => (
                      <li key={i}>
                        ...
                        <span className="text-slate-400">
                          · {c.urgency} urgency · {c.estimatedMinutes}min
                        </span>
                      </li>
                    ))}
                  </ul>
